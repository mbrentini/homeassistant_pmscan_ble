...
from .const import (
    ...
    INDICATE_60S,
    START_CHAR,
    BATTERY_CHAR,          # Gardé pour une éventuelle réactivation plus tard
    RECONNECT_INTERVAL,
    BATTERY_INTERVAL,      # Idem, plus utilisé pour l’instant
)
...
class PMScanCoordinator:
    ...
    async def _connect_and_stream(self) -> None:
        ...
        async with BleakClient(self.address, timeout=15.0) as client:
            ...
            # IMPORTANT : on NE lit plus la batterie ici pour ne rien casser
            # await self._read_battery(client)

            # Notifications PM sur la caractéristique 60s
            ...
            # Boucle de maintien de connexion (sans lecture batterie périodique)
    ...
    def _handle_pm(self, sender, data: bytearray) -> None:
        ...
        [15:17]RH   raw ×0.1 -> 1.1768x - 4.727 + rh_offset
    ...
    async def _read_battery(self, client: BleakClient) -> None:
        """Batterie désactivée pour l’instant (ne lit rien)."""
        return
