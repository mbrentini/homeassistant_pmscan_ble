...
class PMScanCoordinator:
    ...
    async def async_start(self) -> None:
        ...
        self._task = self.hass.async_create_background_task(
            self._run_loop(), name=f"pmscan_{self.address}"
        )
    ...
    async def _connect_and_stream(self) -> None:
        ...
        async with BleakClient(self.address, timeout=15.0) as client:
            ...
            # Battery reading is disabled for stability (see README).
            ...
            while not self._stop_event.is_set() and client.is_connected:
                ...
                # No periodic battery read for now.

    @callback
    def _handle_pm(self, sender, data: bytearray) -> None:
        """Decode PM frame from PMScan (NextPM)."""
        raw = bytes(data)

        if len(raw) < 20 or raw[5] != 0x11:
            _LOGGER.debug("PMScan: RAW ignored (%s)", raw.hex(" ").upper())
            return
    ...
    async def _read_battery(self, client: BleakClient) -> None:
        """Battery reading disabled for now (reserved for a future version)."""
        return
